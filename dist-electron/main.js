...
function setupDatabase() {
    ...
    const dbPath = path_1.default.join(userDataPath, 'vibe_music.db');
...
// --- IPC Handlers ---
// FR-1: Login
electron_1.ipcMain.handle('auth-login', (event, { username, password }) => {
...
electron_1.ipcMain.handle('auth-register', (event, { username, password }) => {
...
electron_1.ipcMain.handle('library-scan', async (event) => {
    ...
    const audioFiles = allFiles.filter((f) => /\.(mp3|flac|wav|ogg)$/i.test(f));
    let addedCount = 0;
    ...
    for (const file of audioFiles) {
        try {
            ...
            const id = (0, uuid_1.v4)();
            insertTrack.run(id, file, metadata.common.title || path_1.default.basename(file), metadata.common.artist || 'Unknown Artist', metadata.common.album || 'Unknown Album', metadata.format.duration || 0, metadata.format.container);
            addedCount++;
        }
        catch (e) {
...
electron_1.ipcMain.handle('library-get-all', () => {
    const stmt = db.prepare('SELECT * FROM tracks ORDER BY artist, album, title');
    return stmt.all();
});
// FR-6: Playlist Management
electron_1.ipcMain.handle('playlist-create', (event, { name }) => {
    const id = (0, uuid_1.v4)();
...
electron_1.ipcMain.handle('playlist-get-tracks', (event, { playlistId }) => {
